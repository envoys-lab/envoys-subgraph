...
import { updatePairDayData, updateTokenDayData, updateEnvoysDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  envoys.totalLiquidityBNB = envoys.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  envoys.totalLiquidityBNB = envoys.totalLiquidityBNB.plus(trackedLiquidityBNB);
  envoys.totalLiquidityUSD = envoys.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  envoys.totalTransactions = envoys.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let envoys = EnvoysFactory.load(FACTORY_ADDRESS);
  envoys.totalVolumeUSD = envoys.totalVolumeUSD.plus(trackedAmountUSD);
  envoys.totalVolumeBNB = envoys.totalVolumeBNB.plus(trackedAmountBNB);
  envoys.untrackedVolumeUSD = envoys.untrackedVolumeUSD.plus(derivedAmountUSD);
  envoys.totalTransactions = envoys.totalTransactions.plus(ONE_BI);
  ...
  let envoysDayData = updateEnvoysDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  envoysDayData.dailyVolumeUSD = envoysDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  envoysDayData.dailyVolumeBNB = envoysDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  envoysDayData.dailyVolumeUntracked = envoysDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  envoysDayData.save();
